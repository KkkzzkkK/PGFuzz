...
def change_wind_dir(value):
	...
	print("[cmd] Random user command: change wind direction %d" %wind_dir)
...
def change_wind(value):
	...
	print("[cmd] Random user command: change wind %d" %rand_wind)
...
def check_stable():
	...
	while True:
		if stable_counter > 0:
			...
			print("\n### Roll error:%f, pitch error:%f, heading error:%f, alt error:%f, counter:%d, flight mode:%s ###\n" %(roll_error, pitch_error, heading_error, alt_error, stable_counter, current_flight_mode))
			stable_counter = 0
			alt_error = 0
			roll_error = 0
			pitch_error = 0
...
def calculate_distance():
	...
	flight_distance = 1 if get_target_flight_mode == Target_flight_mode else -1
	"""
...
def randomly_pick_up_cmd(mandatory):
	...
	for num in range(1, mandatory):
		...
		if num == 1:
			if current_flight_mode != "ALT_HOLD":
				...
				if target_alt_direction == 0: # up direction
					print("[cmd] Random user command: change throttle (up direction)")
					rand_value = random.randint(1600, 1650)

				elif target_alt_direction == 1: # down direction
					print("[cmd] Random user command: change throttle (down direction)")
		...
		# MAV_CMD_NAV_TAKEOFF
			...
			print("[cmd] Random user command: takeoff")
		...
		# Change flight mode
			...
			print("[cmd] Random user command: change flight mode")
			...
			if rand_fligh_mode == Target_flight_mode:
				get_target_flight_mode = Target_flight_mode
			...
			# if a targeted flight mode is triggered at least one time, keep trigger the flight mode
		...
		#-------------------------------------------------------
			...
			print("[cmd] Random user command: change roll angle")
			# Change roll 
	...
	# Pick non-mandatory user commands up
	for x in range(1, 2):
		...
		#-------------------------------------------------------
			...
			print("[cmd] Random user command: add a waypoint")
		...
		#-------------------------------------------------------
			...
			print("[cmd] Random user command: change pitch angle")
	                # Change pitch
		...
		#-------------------------------------------------------
	                ...
	                print("[cmd] Random user command: change yaw angle")
	                # Change yaw
		...
		#-------------------------------------------------------
			...
			print("[cmd] Random user command: Loiter around this waypoint an unlimited amount of time");		
		...
		#-------------------------------------------------------
			...
			print("[cmd] Random user command: set home")
			master.mav.command_long_send(
            			master.target_system,  # target_system
		        	master.target_component, # target_component
			...
			# Log mutated user command
                        ...
                        print_cmd += "set_home"
                        print_cmd += " "
                        print_cmd += str(current_altitude)
                        print_cmd += "\n"
                        write_log(print_cmd)
...
# Parsing parameters
print("#-----------------------------------------------------------------------------")
read_meta_parameter.parsing_parameter('meta_parameters.txt')
...
while True:
	...
	# if AP is still active state
	if drone_status == 4 and executing_commands == 0:
		executing_commands = 1
		print("### Next round (%d) for fuzzing commands. ###" %count_main_loop)
		# Random commands
		randomly_pick_up_cmd(5)
		count_main_loop+=1
		change_parameter()		

		if math.ceil(current_altitude) <= math.ceil(home_altitude):
			print("### Vehicle makes a landing, Home alt:%f, Current alt:%f" %(home_altitude, current_altitude))
			re_launch()
			count_main_loop = 0
